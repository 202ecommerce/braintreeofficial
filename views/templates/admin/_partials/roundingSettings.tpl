...
<ul>
  <li>
    {if $variant == 'help'}<p class='h4'>{/if}
      {l s='Round mode: "Round up away from zero, when it is half way there (recommended) "' mod='braintreeofficial'}
    {if $variant == 'help'}</p>{/if}
  </li>
  <li>
    {if $variant == 'help'}<p class='h4'>{/if}
      {l s='Round type: "Round on each item"' mod='braintreeofficial'}
    {if $variant == 'help'}</p>{/if}
  </li>
  <li>
    {if $variant == 'help'}<p class='h4'>{/if}
...
{if $variant != 'help'}
  ...
  <button class="btn btn-default" data-update-rounding-settings>
    {l s='Change rounding settings automatically' mod='braintreeofficial'}
  </button>
{/if}
