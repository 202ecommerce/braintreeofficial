...
class AdminBraintreeOfficialCustomizeCheckoutController extends AdminBraintreeOfficialController
{
    ...
    public function initBehaviorForm()
    {
        ...
        $this->fields_form['form']['form'] = [
            'legend' => [
                'title' => $this->module->l('Behavior'),
                'icon' => 'icon-cogs',
            ],
            'input' => [
                [
                    'type' => 'switch',
                    'label' => $this->module->l('Accept PayPal Payments'),
                ...
                [
                    'type' => 'html',
                    'label' => $this->module->l('PayPal Express Checkout Shortcut on'),
                    'hint' => $this->module->l('The PayPal Shortcut is displayed directly on your cart or on your product pages, allowing a faster checkout for your buyers. PayPal provides you with the client\'s shipping and billing information so that you don\'t have to collect it yourself.'),
                    'name' => '',
                    'html_content' => $htmlContent,
                ],
                [
                    'type' => 'switch',
                    'label' => $this->module->l('Show PayPal benefits to your customers'),
                    'name' => 'braintreeofficial_show_paypal_benefits',
                    'hint' => $this->module->l('You can increase your conversion rate by presenting PayPal benefits to your customers on payment methods selection page.'),
                    'is_bool' => true,
                    'values' => [
                        [
                ...
                [
                    'type' => 'switch',
                    'label' => $this->module->l('Enable Vault'),
                    'name' => 'braintreeofficial_vaulting',
                    'is_bool' => true,
                    'hint' => $this->module->l('The Vault is used to process payments so your customers don\'t need to re-enter their information each time they make a purchase from you.'),
                    'values' => [
                        [
                ...
                [
                    'type' => 'switch',
                    'label' => $this->module->l('Activate 3D Secure for Braintree'),
                ...
                [
                    'type' => 'text',
                    'label' => $this->module->l('Amount for 3DS in ') . Currency::getCurrency(Configuration::get('PS_CURRENCY_DEFAULT'))['iso_code'],
                    'name' => 'braintreeofficial_3DSecure_amount',
                    'hint' => $this->module->l('Activate 3D Secure only for orders which total is bigger that this amount in your context currency'),
                ],
            ],
            'submit' => [
    ...
    public function initAdvancedForm()
    {
        ...
        $inputs[] = [
            'type' => 'html',
            'name' => '',
            'html_content' => $this->module->displayInformation($this->module->l('You can customize your orders\' status for each possible action in the Braintree module.'), false),
        ];

        $inputs[] = [
            'type' => 'switch',
            'label' => $this->module->l('Customize your order status'),
            'name' => 'braintreeofficial_customize_order_status',
            'hint' => $this->module->l('Please use this option only if you want to change the assigned default Braintree status on PrestaShop Order statuses.'),
            'is_bool' => true,
            'values' => [
                [
        ...
        if (Configuration::get('BRAINTREEOFFICIAL_API_INTENT') == 'authorization') {
            $inputs[] = [
                'type' => 'select',
                'label' => $this->module->l('Payment accepted via BO (call Braintree to get the payment)'),
                ...
                'desc' => $this->module->l('Default status : Payment accepted'),
                'options' => [
                    'query' => $orderStatuses,
                    'id' => 'id',
                    'name' => 'name',
            ...
            $inputs[] = [
                'type' => 'select',
                'label' => $this->module->l('Payment canceled via BO (call Braintree to cancel the capture)'),
        ...
        if (Configuration::get('BRAINTREEOFFICIAL_API_INTENT') == 'sale') {
            $inputs[] = [
                'type' => 'select',
                'label' => $this->module->l('Payment accepted and transaction completed'),
            ...
            $inputs[] = [
                'type' => 'select',
                'label' => $this->module->l('Payment processing (payments by PayPal)'),
                'name' => 'braintreeofficial_os_processing',
                'hint' => $this->module->l('The transaction paid by card can be in the pending status. If the payment is processing the order will be created in the temporary status.'),
                'desc' => $this->module->l('Default status : Payment processing (authorized)'),
                'options' => [
                    'query' => $orderStatuses,
                    'id' => 'id',
                    'name' => 'name',
        ...
        if (Configuration::get('BRAINTREEOFFICIAL_API_INTENT') == 'authorization') {
            $inputs[] = [
                'type' => 'select',
                'label' => $this->module->l('Payment pending (authorized)'),
                'name' => 'braintreeofficial_os_pending',
                'hint' => $this->module->l('You are currently using the Authorize mode. It means that you separate the payment authorization from the capture of the authorized payment. By default the orders will be created in the "Payment pending (authorized)" status but you can customize it if needed.'),
                'desc' => $this->module->l('Default status : Payment pending (authorized)'),
                'options' => [
                    'query' => $orderStatuses,
                    'id' => 'id',
                    'name' => 'name',
        ...
        $this->fields_form['form']['form'] = [
            'legend' => [
                'title' => $this->module->l('Advanced mode'),
                'icon' => 'icon-cogs',
            ],
            'input' => $inputs,
            'submit' => [
